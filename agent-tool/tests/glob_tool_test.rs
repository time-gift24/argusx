...
async fn glob_honors_max_depth_and_max_results() {
    ...
    assert!(result.output["results"].as_array().unwrap().iter().all(|r| {
        r["path"].as_str().unwrap().contains("root.txt")
    }));
...
async fn glob_honors_min_max_bytes_filters() {
    ...
    std::fs::write(temp_dir.path().join("large.txt"), "this is a larger file content").unwrap();
    ...
    assert!(!result.is_error);
    assert_eq!(result.output["count"].as_i64().unwrap(), 1);
    assert!(result.output["results"].as_array().unwrap()[0]["path"].as_str().unwrap().contains("large.txt"));
    ...
    assert!(!result.is_error);
    assert_eq!(result.output["count"].as_i64().unwrap(), 1);
    assert!(result.output["results"].as_array().unwrap()[0]["path"].as_str().unwrap().contains("small.txt"));
}
