...
pub enum Error {
    #[error("parse error: {0}")]
    Parse(#[from] crate::dialect::zai::parser::Error),
...
impl Mapper {
    ...
    pub fn feed(&mut self, raw: &str) -> Result<Vec<ResponseEvent>, Error> {
        ...
        match parse_payload(raw)? {
            ZaiStreamEvent::Chunk(chunk) => self.process_chunk(chunk),
            ZaiStreamEvent::Open => Ok(Vec::new()),
            ZaiStreamEvent::Done => Err(Error::Protocol(
                "received [DONE] in feed(); call on_done() instead".into(),
            )),
            ZaiStreamEvent::Error(err) => {
                Err(Error::Protocol(format!("upstream stream error: {err}")))
            }
        }
    }

    fn process_chunk(&mut self, chunk: ZaiStreamChunk) -> Result<Vec<ResponseEvent>, Error> {
        let ZaiStreamChunk {
            id,
            object,
            created,
        ...
        if !self.created {
            let created = i64::try_from(created)
                .map_err(|_| Error::Protocol(format!("created is out of i64 range: {created}")))?;
