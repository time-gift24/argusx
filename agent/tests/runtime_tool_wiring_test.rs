...
fn mock_stream(
) -> Pin<Box<dyn Stream<Item = Result<agent_core::ModelOutputEvent, agent_core::AgentError>> + Send>>
{
    use futures::stream;
...
async fn agent_builder_accepts_agent_center() -> anyhow::Result<()> {
    ...
    let center = Arc::new(AgentCenter::builder().db_path(db_path).build()?);
    ...
    let session_id = agent
        .create_session(None, Some("Test Session".to_string()))
        .await?;
...
async fn agent_builder_works_without_agent_center() -> anyhow::Result<()> {
    ...
    let model = Arc::new(MockModel);
    let agent = AgentBuilder::new().model(model).build().await?;
