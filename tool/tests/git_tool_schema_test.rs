...
fn git_args_status_parses_correctly() {
    ...
    match args {
        tool::builtin::git::types::GitArgs::Status { repo_path, include_untracked } => {
...
fn git_args_commit_with_message_parses() {
    ...
    match args {
        tool::builtin::git::types::GitArgs::Commit { repo_path, message, allow_empty } => {
            assert_eq!(repo_path, "/tmp/repo");
...
fn git_tool_schema_has_one_of_with_all_actions() {
    ...
    assert!(action_names.contains(&"branch_list"), "missing branch_list action");
    assert!(action_names.contains(&"remote_list"), "missing remote_list action");
    assert!(action_names.contains(&"worktree_list"), "missing worktree_list action");
    assert!(action_names.contains(&"add"), "missing add action");
    assert!(action_names.contains(&"commit"), "missing commit action");
    assert!(action_names.contains(&"branch_create"), "missing branch_create action");
    assert!(action_names.contains(&"checkout"), "missing checkout action");
...
fn git_tool_schema_commit_action_has_required_fields() {
    ...
    let required = commit_action.get("required").expect("commit should have required fields");
    let required_arr = required.as_array().expect("required should be an array");

    assert!(required_arr.contains(&json!("action")), "action should be required");
    assert!(required_arr.contains(&json!("repo_path")), "repo_path should be required");
    assert!(required_arr.contains(&json!("message")), "message should be required");
}
